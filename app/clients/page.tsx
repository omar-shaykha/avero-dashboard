...
export default function ClientsPage() {
  ...
  useEffect(()=>{const load=async()=>{try{const userRes=await fetch("/api/auth/user"); if(!userRes.ok){router.push("/login");return;} const userData=await userRes.json();setUser(userData); const profileRes=await fetch("/api/auth/profile"); if(!profileRes.ok){setAccessDenied(true);setLoading(false);return;} const profileData=await profileRes.json(); if(profileData.role!=="king_admin"){setAccessDenied(true);setLoading(false);return;} setUserName(profileData.full_name||profileData.nickname||profileData.username||userData.email?.split("@")[0]||""); const companiesRes=await fetch("/api/clients"); if(!companiesRes.ok){setAccessDenied(companiesRes.status===403);setLoading(false);return;} setCompanies(await companiesRes.json());setLoading(false);}catch(error){console.error("Error loading clients page:",error);setAccessDenied(true);setLoading(false);}};load();},[router]);
