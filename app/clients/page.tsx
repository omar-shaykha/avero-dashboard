...
export default function ClientsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (!profileRes.ok) { setAccessDenied(true); setLoading(false); return; }
        const profileData = await profileRes.json();
        if (profileData.role !== "king_admin") { setAccessDenied(true); setLoading(false); return; }
        setUserName(profileData.full_name || profileData.nickname || profileData.username || userData.email?.split("@")[0] || "");
        const companiesRes = await fetch("/api/clients");
        if (!companiesRes.ok) { setAccessDenied(companiesRes.status === 403); setLoading(false); return; }
  ...
  const handleAddClient = async (formData: AddClientForm) => {
    ...
    if (!response.ok) {
      const prefix = response.status === 409 ? (ar ? "تعارض" : "Conflict") : response.status === 400 ? (ar ? "بيانات غير صالحة" : "Invalid input") : (ar ? "خطأ" : "Error");
