...
import CrmAgentFloor from "@/app/components/CrmAgentFloor";
...
export default async function SalesCrmPage() {
  ...
  const { data: allLeads, error } = await supabase.from("leads").select(`id,title,service_type,status,interest_level,estimated_value,people_count,appointment_date,city,notes,updated_at,lead_source,priority,next_follow_up_at,next_action,probability,last_contact_at,tags,custom_fields,customers(name,phone,email)`).eq("company_id", companyId).order("updated_at", { ascending:false });
  ...
  return <div className="min-h-screen bg-slate-950"><Sidebar userEmail={user.email} userName={userName} access={access}/><div className="ml-64 flex min-h-screen flex-col"><DashboardHeader userEmail={user.email} userName={userName}/><div className="px-6 pt-6"><LocalizedPageHeader enEyebrow="AVERO CRM / ALL AGENTS" arEyebrow="AVERO CRM / جميع الوكلاء" enTitle="CRM" arTitle="CRM"/><div className="mt-4 flex flex-wrap gap-2">{[["ai_sales","Leo"],["ai_marketing","Foxy"],["ai_hr","Aero"],["ai_support","Gor"],["ai_inventory","Vexa"],["ai_warehouse","Bruno"],["ai_customer_care","Rex"],["ai_analytics","Nova"]].map(([key,name])=><Link key={key} href={"/crm/agents/"+key} className="rounded-xl border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-black text-cyan-300 hover:border-cyan-500">{name+" CRM"}</Link>)}</div></div><CrmAgentFloor/><SearchToolbar/><StatsCards totalLeads={leads.length} qualifiedCount={leads.filter(l=>l.status==="qualified").length} quotationsCount={leads.filter(l=>l.status==="quotation").length} negotiationsCount={leads.filter(l=>l.status==="negotiation").length} wonCount={leads.filter(l=>l.status==="won").length} lostCount={leads.filter(l=>l.status==="lost").length}/><div className="flex-1 px-6 py-6"><LeadPipeline leads={leads}/></div></div></div>;
}
