...
export async function POST(request: Request) {
  try {
    ...
    const looksLikeAction = /\b(change|update|enable|disable|turn on|turn off|edit|modify|activate|deactivate)\b/i.test(message)
    ...
    if (looksLikeAction) {
      const made = await ctx.admin
        ...
        .insert({
          ...
          command_type: "user_requested_change",
          command_text: message,
          status: "awaiting_confirmation",
          requires_confirmation: true,
          payload: { source: "help_center", session_id: sessionId },
        })
        .select("id,status,requires_confirmation,created_at")
        .single();
