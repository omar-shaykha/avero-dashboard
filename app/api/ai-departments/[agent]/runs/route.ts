...
export async function GET(_: Request, { params }: { params: Promise<{ agent: string }> }) {
  try {
    ...
    const { data, error } = await db()
      .from("ai_agent_runs")
      .select("id,agent_key,action,status,input,output,error_message,created_at,completed_at")
      .eq("company_id", companyId)
      .eq("agent_key", a.key)
      .order("created_at", { ascending: false })
      .limit(12);
    if (error) return Response.json({ error: "Failed to load live runs" }, { status: 500 });
    return Response.json({ agent: a.key, runs: data || [] });
  } catch (error) {
    console.error("AI runs GET error:", error);
