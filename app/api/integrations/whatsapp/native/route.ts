...
async function sendWhatsApp(s: SupabaseClient, phoneNumberId: string, to: string, body: string) {
  const token = await getSecret(s, "meta_whatsapp_access_token");
  if (!token) throw new Error("Meta WhatsApp access token is not connected");
...
export async function GET(request: Request) {
  try {
    ...
    const expected = await getSecret(s, "meta_whatsapp_verify_token");
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!text) return Response.json({ ok: true, ignored: true, reason: "Only text is enabled in Leo native v1" });
    ...
    if (!customer) {
      const inserted = await s.from("customers").insert({
      ...
      await s.from("customers").update({ last_message_at: new Date().toISOString() }).eq("id", customer.id);
    }
    ...
    const sent = await sendWhatsApp(s, phoneNumberId, from, reply);
    ...
    await s.from("ai_agent_runs").insert({
      ...
      status: "completed",
      input: { customer_id: customer.id, message_id: externalMessageId, message: text },
      output: { customer_id: customer.id, lead_id: lead?.id || null, reply, sent_message_id: sentId, source: "vercel_native_vault" },
    ...
    console.error("Leo native WhatsApp error", error);
