...
export async function GET(){
 ...
 const [customers,b2b,invoices,payments,tables,invoiceTemplate,products,categories,profiles,warehouses,suppliers,items,units]=await Promise.all([
  ...
  x.s.from('sales_products').select('*,sales_categories(name),inventory_items(name,sku,average_cost)').eq('company_id',x.c).eq('active',true).order('created_at',{ascending:false}),
...
export async function POST(req:Request){
 ...
 if(b.kind==='customer'){
  ...
  if(!can(x.a,'b2b.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const a=await x.s.from('b2b_accounts').select('id,payment_terms_days').eq('id',d.account_id).eq('company_id',x.c).maybeSingle();
  if(!a.data)return NextResponse.json({error:'Invalid B2B account'},{status:400});
  const lines=Array.isArray(d.lines)?d.lines.filter((z:any)=>String(z.description||'').trim()&&Number(z.quantity)>0):[];
  if(!lines.length)return NextResponse.json({error:'Add invoice lines'},{status:400});
  ...
  const inv=await x.s.from('b2b_invoices').insert({company_id:x.c,account_id:d.account_id,invoice_no:d.invoice_no||`B2B-${Date.now()}`,invoice_date:d.invoice_date||new Date().toISOString().slice(0,10),due_date:d.due_date||null,status:d.status||'issued',subtotal,tax,total:subtotal+tax,notes:d.notes||null,created_by:x.a.user.id}).select().single();
  if(inv.error)return NextResponse.json({error:inv.error.message},{status:400});
  const lr=await x.s.from('b2b_invoice_lines').insert(lines.map((z:any)=>{const base=Number(z.quantity)*Number(z.unit_price||0);return{company_id:x.c,invoice_id:inv.data.id,description:String(z.description).trim(),quantity:Number(z.quantity),unit_price:Number(z.unit_price||0),tax_rate:Number(z.tax_rate||0),line_total:base+base*Number(z.tax_rate||0)/100}}));
  if(lr.error){await x.s.from('b2b_invoices').delete().eq('id',inv.data.id);return NextResponse.json({error:lr.error.message},{status:400});}
  return NextResponse.json({record:inv.data});
 }
 else if(b.kind==='payment_method'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const pct=Math.max(0,Math.min(100,Number(d.adjustment_percent||0)));
  const p={company_id:x.c,code:String(d.code||d.name||'pay').toLowerCase().replace(/\s+/g,'_'),name:String(d.name||'Payment'),adjustment_percent:pct,adjustment_type:d.adjustment_type==='discount'?'discount':'markup',active:d.active!==false,sort_order:Number(d.sort_order||0),updated_at:new Date().toISOString()};
  ...
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const p={company_id:x.c,name:String(d.name||'Table').trim(),area:d.area||null,seats:Math.max(1,Number(d.seats||2)),status:d.status||'open',sort_order:Number(d.sort_order||0),updated_at:new Date().toISOString()};
  r=d.id?await x.s.from('sales_dining_tables').update(p).eq('id',d.id).eq('company_id',x.c).select().single():await x.s.from('sales_dining_tables').insert(p).select().single();
 }
 else if(b.kind==='invoice_template'){
  if(!can(x.a,'sales.invoice.customize'))return NextResponse.json({error:'Forbidden'},{status:403});
  r=await x.s.from('sales_invoice_templates').upsert({company_id:x.c,business_name:d.business_name||null,logo_url:d.logo_url||null,commercial_registration:d.commercial_registration||null,vat_number:d.vat_number||null,address:d.address||null,phone:d.phone||null,header_text:d.header_text||null,footer_text:d.footer_text||null,paper_size:['roll58','roll80','a4'].includes(d.paper_size)?d.paper_size:'roll80',show_tax_number:d.show_tax_number!==false,show_cashier:d.show_cashier!==false,show_customer:d.show_customer!==false,show_item_notes:d.show_item_notes!==false,show_cr:d.show_cr!==false,show_vat:d.show_vat!==false,show_address:d.show_address!==false,show_phone:d.show_phone!==false,accent:d.accent||'#06b6d4',updated_at:new Date().toISOString()},{onConflict:'company_id'}).select().single();
 }
 else if(b.kind==='generate_product_code'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const g=await x.s.rpc('sales_generate_code',{p_company_id:x.c,p_prefix:'PRD'});
  if(g.error)return NextResponse.json({error:g.error.message},{status:400});
  return NextResponse.json({code:g.data,barcode:g.data});
 }
 else if(b.kind==='product_save'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const name=String(d.name||'').trim();
  const price=Number(d.price),cost=Number(d.cost||0),openingQty=Number(d.opening_quantity||0);
  const unit=['piece','gram','kilogram','liter'].includes(d.sale_unit)?d.sale_unit:'piece';
  if(!name)return NextResponse.json({error:'Product name is required'},{status:400});
  if(!Number.isFinite(price)||price<0)return NextResponse.json({error:'Price must be zero or greater'},{status:400});
  if(!Number.isFinite(cost)||cost<0)return NextResponse.json({error:'Cost must be zero or greater'},{status:400});
  if(!Number.isFinite(openingQty)||openingQty<0)return NextResponse.json({error:'Opening quantity cannot be negative'},{status:400});
  ...
  if(d.supplier_id&&!await owned(x.s,'suppliers',d.supplier_id,x.c))return NextResponse.json({error:'Invalid supplier'},{status:400});
  if(openingQty>0&&!await owned(x.s,'inventory_warehouses',d.warehouse_id,x.c))return NextResponse.json({error:'Select a valid warehouse for opening stock'},{status:400});

  let sku=String(d.sku||'').trim().toUpperCase();
  if(!sku){const g=await x.s.rpc('sales_generate_code',{p_company_id:x.c,p_prefix:'PRD'});if(g.error)return NextResponse.json({error:g.error.message},{status:400});sku=g.data;}
  const barcode=String(d.barcode||'').trim()||sku;
  const track=d.track_inventory!==false;
  let inventoryItemId=d.inventory_item_id||null;

  try{
   if(track&&!inventoryItemId){
    const unitId=await ensureUnit(x,unit);
    const ii=await x.s.from('inventory_items').insert({company_id:x.c,sku,barcode,name,item_type:'finished_good',category:d.category_name||null,base_unit_id:unitId,purchase_unit_id:unitId,purchase_to_base_factor:1,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,costing_method:'weighted_average',standard_cost:cost,average_cost:0,last_purchase_cost:cost,min_stock:0,max_stock:0,reorder_point:0,safety_stock:0,track_batches:false,track_expiry:false,track_serials:false,allow_negative_stock:false,active:true,created_by:x.a.user.id,updated_by:x.a.user.id}).select('id').single();
    if(ii.error)return NextResponse.json({error:ii.error.message},{status:400});
    inventoryItemId=ii.data.id;
   ...
   const productPayload:any={company_id:x.c,category_id:d.category_id||null,inventory_item_id:track?inventoryItemId:null,preferred_supplier_id:d.supplier_id||null,sku,barcode,name,description:d.description||null,image_url:d.image_url||null,image_path:null,sale_unit:unit,weight:null,price,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,track_inventory:track,show_on_cashier:d.show_on_cashier!==false,active:true,sort_order:Number(d.sort_order||0),updated_at:new Date().toISOString()};
   let pr;
   if(d.id)pr=await x.s.from('sales_products').update(productPayload).eq('id',d.id).eq('company_id',x.c).select().single();
   else pr=await x.s.from('sales_products').insert(productPayload).select().single();
   if(pr.error)return NextResponse.json({error:pr.error.message},{status:400});

   if(track&&inventoryItemId){
    const unitId=await ensureUnit(x,unit);
    await x.s.from('inventory_items').update({name,sku,barcode,base_unit_id:unitId,purchase_unit_id:unitId,standard_cost:cost,last_purchase_cost:cost,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,updated_at:new Date().toISOString(),updated_by:x.a.user.id}).eq('id',inventoryItemId).eq('company_id',x.c);
    if(d.supplier_id){
      await x.s.from('inventory_item_suppliers').upsert({company_id:x.c,item_id:inventoryItemId,supplier_id:d.supplier_id,purchase_unit_id:unitId,conversion_factor:1,last_price:cost,minimum_order_qty:0,lead_time_days:0,preferred:true,active:true},{onConflict:'company_id,item_id,supplier_id'});
    }
    if(!d.id&&openingQty>0){
      const mv=await x.s.rpc('inventory_apply_delta',{p_company_id:x.c,p_item_id:inventoryItemId,p_warehouse_id:d.warehouse_id,p_location_id:null,p_delta:openingQty,p_unit_cost:cost,p_movement_type:'opening_stock',p_reference_type:'product_setup',p_reference_id:pr.data.id,p_reference_no:sku,p_reason:'Opening stock from product setup',p_created_by:x.a.user.id});
      if(mv.error)return NextResponse.json({error:mv.error.message},{status:400});
    }
   }
   return NextResponse.json({record:pr.data});
  }catch(e:any){return NextResponse.json({error:e?.message||'Could not save product'},{status:400});}
 }
 else if(b.kind==='product_delete'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const pr=await x.s.from('sales_products').update({active:false,show_on_cashier:false,updated_at:new Date().toISOString()}).eq('id',d.id).eq('company_id',x.c).select('id').maybeSingle();
  if(pr.error)return NextResponse.json({error:pr.error.message},{status:400});
  if(!pr.data)return NextResponse.json({error:'Product not found'},{status:404});
