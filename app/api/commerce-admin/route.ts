...
async function ensureOpeningWarehouse(x:any,requested?:string){
 if(requested){if(await owned(x.s,'inventory_warehouses',requested,x.c))return requested;throw new Error('Invalid opening stock warehouse');}
 ...
 if(ins.error){const retry=await x.s.from('inventory_warehouses').select('id').eq('company_id',x.c).eq('code','MAIN').maybeSingle();if(retry.data?.id)return retry.data.id;throw new Error(ins.error.message);}
 return ins.data.id;
}
async function existsOther(x:any,table:string,field:string,value:string,excludeId?:string|null){if(!value)return false;let q=x.s.from(table).select('id').eq('company_id',x.c).eq(field,value).limit(1);if(excludeId)q=q.neq('id',excludeId);const r=await q;return !!r.data?.length}
async function codeTaken(x:any,value:string,productId?:string|null,inventoryItemId?:string|null){if(!value)return false;const v=String(value).trim().toUpperCase();const checks=await Promise.all([
 existsOther(x,'sales_products','sku',v,productId),existsOther(x,'sales_products','barcode',v,productId),
 existsOther(x,'inventory_items','sku',v,inventoryItemId),existsOther(x,'inventory_items','barcode',v,inventoryItemId)
...
export async function GET(){
 ...
 const [customers,b2b,invoices,payments,tables,invoiceTemplate,products,categories,profiles,warehouses,suppliers,items,units,recipes]=await Promise.all([
  ...
  x.s.from('sales_products').select('*,sales_categories(name),inventory_items(id,name,sku,average_cost,item_type,purchasable)').eq('company_id',x.c).eq('active',true).order('created_at',{ascending:false}),
  ...
  x.s.from('inventory_items').select('id,name,sku,average_cost,item_type,purchasable').eq('company_id',x.c).eq('active',true).order('name'),
...
export async function POST(req:Request){
 ...
 if(b.kind==='category_save'){
  ...
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});const t=TYPES.includes(d.product_type)?d.product_type:'purchased_product';const prefix=t==='recipe_product'?'RCP':t==='sub_recipe'?'SUB':'PRD';const g=await x.s.rpc('sales_generate_code',{p_company_id:x.c,p_prefix:prefix});if(g.error)return NextResponse.json({error:g.error.message},{status:400});return NextResponse.json({code:g.data,barcode:g.data});
 }
 else if(b.kind==='product_save'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});
  const name=String(d.name||'').trim(),price=Number(d.price),cost=Number(d.cost||0),openingQty=Number(d.opening_quantity||0),unit=['piece','gram','kilogram','liter','milliliter'].includes(d.sale_unit)?d.sale_unit:'piece';
  ...
  if(productType==='raw_material'){policy='stock';d.recipe_id=null;d.sub_recipe_id=null;}if(productType==='non_stock'){policy='none';d.recipe_id=null;d.sub_recipe_id=null;}if(productType==='purchased_product'){if(!['stock','none'].includes(policy))policy='stock';d.recipe_id=null;d.sub_recipe_id=null;}
  const track=policy==='stock'||policy==='produced_stock';const show=productType==='sub_recipe'?false:d.show_on_cashier!==false;const purchasable=track&&d.purchasable===true;
  if(openingQty>0&&!track)return NextResponse.json({error:'Opening quantity is only valid for inventory-tracked products'},{status:400});
  let openingWarehouseId:string|null=null;try{if(openingQty>0)openingWarehouseId=await ensureOpeningWarehouse(x,d.warehouse_id||undefined);}catch(e:any){return NextResponse.json({error:e.message},{status:400});}
  let sku=String(d.sku||'').trim().toUpperCase();if(!sku){const g=await x.s.rpc('sales_generate_code',{p_company_id:x.c,p_prefix:productType==='recipe_product'?'RCP':productType==='sub_recipe'?'SUB':'PRD'});if(g.error)return NextResponse.json({error:g.error.message},{status:400});sku=String(g.data).trim().toUpperCase();}let barcode=String(d.barcode||'').trim().toUpperCase()||sku;
  if(await codeTaken(x,sku,d.id||null,d.inventory_item_id||null))return NextResponse.json({error:'Product code already exists. Generate a new code.'},{status:409});
  if(barcode!==sku&&await codeTaken(x,barcode,d.id||null,d.inventory_item_id||null))return NextResponse.json({error:'Barcode already exists. Use a different barcode.'},{status:409});
  let inventoryItemId=d.inventory_item_id||null;let createdInventoryItem=false;try{
   if(track&&!inventoryItemId){const unitId=await ensureUnit(x,unit);const itemType=productType==='raw_material'?'raw_material':productType==='sub_recipe'?'sub_recipe_output':'finished_good';const ii=await x.s.from('inventory_items').insert({company_id:x.c,sku,barcode,name,item_type:itemType,category:d.category_name||null,base_unit_id:unitId,purchase_unit_id:unitId,purchase_to_base_factor:1,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,costing_method:'weighted_average',standard_cost:cost,average_cost:0,last_purchase_cost:cost,min_stock:0,max_stock:0,reorder_point:0,safety_stock:0,track_batches:false,track_expiry:false,track_serials:false,allow_negative_stock:false,purchasable,active:true,created_by:x.a.user.id,updated_by:x.a.user.id}).select('id').single();if(ii.error)return NextResponse.json({error:ii.error.message},{status:400});inventoryItemId=ii.data.id;createdInventoryItem=true;}
   ...
   const payload:any={company_id:x.c,category_id:d.category_id||null,inventory_item_id:track?inventoryItemId:d.inventory_item_id||null,preferred_supplier_id:d.supplier_id||null,recipe_id:productType==='recipe_product'?d.recipe_id:null,sub_recipe_id:productType==='sub_recipe'?d.sub_recipe_id:null,product_type:productType,inventory_policy:policy,accounting_class:accountingClass,sku,barcode,name,description:d.description||null,image_url:d.image_url||null,image_path:null,sale_unit:unit,weight:null,price,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,track_inventory:track,show_on_cashier:show,purchasable,active:true,sort_order:Number(d.sort_order||0),updated_at:new Date().toISOString()};
   let pr=d.id?await x.s.from('sales_products').update(payload).eq('id',d.id).eq('company_id',x.c).select().single():await x.s.from('sales_products').insert(payload).select().single();if(pr.error){if(createdInventoryItem&&inventoryItemId)await x.s.from('inventory_items').delete().eq('id',inventoryItemId).eq('company_id',x.c);return NextResponse.json({error:pr.error.message},{status:400});}
   if(track&&inventoryItemId){const unitId=await ensureUnit(x,unit),itemType=productType==='raw_material'?'raw_material':productType==='sub_recipe'?'sub_recipe_output':'finished_good';const iu=await x.s.from('inventory_items').update({name,sku,barcode,item_type:itemType,base_unit_id:unitId,purchase_unit_id:unitId,standard_cost:cost,last_purchase_cost:cost,tax_enabled:!!d.tax_enabled,tax_rate:d.tax_enabled?Number(d.tax_rate||0):0,purchasable,updated_at:new Date().toISOString(),updated_by:x.a.user.id}).eq('id',inventoryItemId).eq('company_id',x.c);if(iu.error)return NextResponse.json({error:iu.error.message},{status:400});
    if(!purchasable)await x.s.from('inventory_item_suppliers').update({preferred:false,active:false}).eq('company_id',x.c).eq('item_id',inventoryItemId);
    if(purchasable&&d.supplier_id){await x.s.from('inventory_item_suppliers').update({preferred:false}).eq('company_id',x.c).eq('item_id',inventoryItemId).neq('supplier_id',d.supplier_id);const old=await x.s.from('inventory_item_suppliers').select('id').eq('company_id',x.c).eq('item_id',inventoryItemId).eq('supplier_id',d.supplier_id).maybeSingle();const sp={company_id:x.c,item_id:inventoryItemId,supplier_id:d.supplier_id,purchase_unit_id:unitId,conversion_factor:1,last_price:cost,minimum_order_qty:0,lead_time_days:0,preferred:true,active:true};if(old.data)await x.s.from('inventory_item_suppliers').update(sp).eq('id',old.data.id);else await x.s.from('inventory_item_suppliers').insert(sp);}
    if(!d.id&&openingQty>0&&openingWarehouseId){const mv=await x.s.rpc('inventory_apply_delta',{p_company_id:x.c,p_item_id:inventoryItemId,p_warehouse_id:openingWarehouseId,p_location_id:null,p_delta:openingQty,p_unit_cost:cost,p_movement_type:'opening_stock',p_reference_type:'product_setup',p_reference_id:pr.data.id,p_reference_no:sku,p_reason:'Opening stock from Product Master',p_created_by:x.a.user.id});if(mv.error)return NextResponse.json({error:mv.error.message},{status:400});}
   }
   return NextResponse.json({record:pr.data,inventory_item_id:inventoryItemId,logic:{product_type:productType,inventory_policy:policy,accounting_class:accountingClass,purchasable}});
  }catch(e:any){if(createdInventoryItem&&inventoryItemId&&!d.id)await x.s.from('inventory_items').delete().eq('id',inventoryItemId).eq('company_id',x.c);return NextResponse.json({error:e.message||'Product sync failed'},{status:400});}
 }
 else if(b.kind==='product_delete'){
  if(!can(x.a,'sales.manage'))return NextResponse.json({error:'Forbidden'},{status:403});r=await x.s.from('sales_products').update({active:false,show_on_cashier:false,purchasable:false,updated_at:new Date().toISOString()}).eq('id',d.id).eq('company_id',x.c).select().single();
 }
