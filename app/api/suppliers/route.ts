...
export async function GET() {
  ...
  if (!can(access, "suppliers.view") && !can(access, "suppliers.manage")) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
...
export async function POST(req: Request) {
  const access = await getAuthorizationContext();
  if (!access?.profile?.company_id) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (!can(access, "suppliers.manage")) return NextResponse.json({ error: "Forbidden" }, { status: 403 });
  ...
  if (body.tax_enabled && (!Number.isFinite(taxRate) || taxRate < 0 || taxRate > 100)) {
  ...
  const payload = {
    ...
    cr_number: body.cr_number || null, tax_enabled: !!body.tax_enabled, tax_rate: body.tax_enabled ? taxRate : 0,
    payment_terms_days: Math.max(0, Number(body.payment_terms_days || 0)), credit_limit: Math.max(0, Number(body.credit_limit || 0)), opening_balance: Number(body.opening_balance || 0),
    currency: body.currency || "SAR", bank_name: body.bank_name || null, iban: body.iban || null, category: body.category || null,
    lead_time_days: Math.max(0, Number(body.lead_time_days || 0)), minimum_order: Math.max(0, Number(body.minimum_order || 0)), preferred: !!body.preferred,
