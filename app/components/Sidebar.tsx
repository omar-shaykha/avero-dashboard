...
export default function Sidebar({ userEmail, userName, access }: SidebarProps) {
  ...
  return (
    <div className={`fixed top-0 z-40 flex h-screen w-64 flex-col border-slate-800/80 bg-[radial-gradient(circle_at_top_left,rgba(37,99,235,0.10),transparent_28%),#020617] ${rtl ? "right-0 border-l" : "left-0 border-r"}`} dir={rtl ? "rtl" : "ltr"}>
      <div className="border-b border-slate-800/80 px-5 py-5"><AveroBrand /></div>
      <nav className="flex-1 space-y-2 overflow-y-auto p-4">
        <Link href="/" className={`flex items-center gap-3 rounded-xl px-4 py-3 transition-all ${pathname === "/" ? "border border-blue-500/25 bg-gradient-to-r from-blue-500/15 to-cyan-400/5 text-blue-300" : "text-slate-300 hover:bg-slate-900/80 hover:text-white"}`}><LayoutDashboard size={18}/><span className="text-sm font-medium">{t("dashboard")}</span></Link>

        {agents.length > 0 && <div><button onClick={()=>setAgentsOpen((v)=>!v)} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-start transition-all ${pathname?.startsWith("/ai-") ? "border border-violet-500/25 bg-gradient-to-r from-violet-500/15 to-blue-400/5 text-violet-300" : "text-slate-300 hover:bg-slate-900/80 hover:text-white"}`}><Bot size={18}/><span className="flex-1 text-sm font-medium">{t("aiAgents")}</span>{agentsOpen ? <ChevronDown size={15}/> : arrow}</button>{agentsOpen && <div className={`mt-2 space-y-1 border-slate-800 ${rtl ? "mr-5 border-r pr-3" : "ml-5 border-l pl-3"}`}>{agents.map((agent)=><Sub key={agent.href} href={agent.href} label={agent.label} pathname={pathname}/>)}</div>}</div>}

        {crmVisible && <div><button onClick={()=>setCrmOpen((v)=>!v)} className={`flex w-full items-center gap-3 rounded-xl px-4 py-3 text-start transition-all ${pathname?.startsWith("/crm") ? "border border-blue-500/25 bg-gradient-to-r from-blue-500/15 to-cyan-400/5 text-blue-300" : "text-slate-300 hover:bg-slate-900/80 hover:text-white"}`}><UsersRound size={18}/><span className="flex-1 text-sm font-medium">{t("crm")}</span>{crmOpen ? <ChevronDown size={15}/> : arrow}</button>{crmOpen && <div className={`mt-2 space-y-1 border-slate-800 ${rtl ? "mr-5 border-r pr-3" : "ml-5 border-l pl-3"}`}><Sub href="/crm" label={t("crmHub")} pathname={pathname} exact/><Sub href="/crm/sales" label={t("salesCrm")} pathname={pathname}/><Sub href="/crm/marketing" label={t("marketingCrm")} pathname={pathname}/><Sub href="/crm/hr" label={t("hrCrm")} pathname={pathname}/><Sub href="/crm/support" label={t("supportCrm")} pathname={pathname}/></div>}</div>}
...
function Sub({href,label,pathname,exact=false}:{href:string;label:string;pathname:string|null;exact?:boolean}) { const active=exact?pathname===href:pathname?.startsWith(href); return <Link href={href} className={`flex items-center gap-2 rounded-lg px-3 py-2 text-xs transition ${active?"bg-blue-500/10 text-blue-300":"text-slate-500 hover:bg-slate-900 hover:text-slate-300"}`}><Sparkles size={12}/>{label}</Link>; }
