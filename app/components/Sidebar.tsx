...
export default function Sidebar({ access }: SidebarProps) {
  ...
  const sections: Record<NavSectionKey, ReactNode> = {
    ...
    add_items: sellVisible && canModule("sales.view", "sales.manage") ? <Main href="/pos?area=add-items" label={collapsed ? "" : L("Add Items", "إضافة الأصناف")} icon={PackagePlus} active={pathname === "/pos" && currentArea === "add-items"} onClick={() => selectRoute("add-items")} /> : null,
