...
type Data = { company: { name: string }; store: Store | null; branches: { id: string; name: string }[]; products: Product[]; orders: GoOrder[]; can_manage: boolean };
...
export default function GoMerchantWorkspace() {
  ...
  const load = useCallback(async () => {
    const response = await fetch("/api/go/manage", { cache: "no-store" });
  ...
  async function change(payload: Record<string, unknown>) {
    ...
    try {
      const response = await fetch("/api/go/manage", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      const result = await response.json();
  ...
  return <div className="mt-8 space-y-7" dir="rtl">
    ...
    <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6">
      <div className="flex flex-wrap items-center justify-between gap-4"><h2 className="text-xl font-bold">٢ · الأصناف والأسعار</h2><Link href="/pos?area=add-items" className="rounded-xl border border-slate-600 px-4 py-2 text-sm text-cyan-300">إضافة الأصناف وتعديل الأسعار ↗</Link></div>
