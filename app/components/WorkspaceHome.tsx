  ...
  return <main className="mx-auto max-w-7xl space-y-8 px-5 py-8 md:px-8">
    ...
    <section aria-labelledby="control-title">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {canViewSettings && <Link href="/settings" className="rounded-2xl border border-slate-800 bg-slate-900/60 p-5 hover:border-cyan-400/40"><Settings2 className="text-cyan-300"/><h3 className="mt-3 font-bold">{L("Company & team", "الشركة والفريق")}</h3><p className="mt-2 text-sm text-slate-400">{L("Company settings, users and permissions", "إعدادات الشركة والمستخدمون والصلاحيات")}</p></Link>}
    ...
    <section aria-labelledby="apps-title">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {products.map(({ key, title, description, href, icon: Icon }) => {
          const enabled = apps[key];
          const content = <><div className="flex items-start justify-between"><Icon size={28} className={enabled ? "text-cyan-300" : "text-slate-500"}/>{enabled ? <ArrowUpRight size={20} className="text-cyan-300"/> : <span className="rounded-full border border-slate-700 px-3 py-1 text-xs text-slate-400">{key === "go" ? L("In preparation", "قيد التجهيز") : L("Not available", "غير متاح")}</span>}</div><h3 className="mt-6 text-xl font-bold">{title}</h3><p className="mt-2 text-sm text-slate-400">{description}</p></>;
          return enabled ? <Link key={key} href={href} className="rounded-2xl border border-slate-700 bg-slate-900 p-6 transition hover:border-cyan-400/50">{content}</Link> : <div key={key} className="rounded-2xl border border-slate-800 bg-slate-900/30 p-6">{content}</div>;
        })}
      </div>
    </section>
