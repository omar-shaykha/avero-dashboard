...
export default function ProductsWorkspace(){
  ...
  function newProduct(){resetImage();resetRecipe();setForm({name:"",price:"",cost:"",opening_quantity:"",category_id:"",section_id:"",sku:"",barcode:"",sale_unit:"piece",supplier_id:"",warehouse_id:"",show_on_cashier:true,purchasable:true,product_type:"purchased_product",inventory_policy:"stock",tax_enabled:true,tax_rate:15,image_url:null,image_path:null,recipe_disabled:false});setOpen(true);}
  function editProduct(p:any){resetImage();setRecipeBuilder(false);setRecipeDraft({name:"",lines:[]});const rp=d.recipes.find((r:any)=>r.id===p.recipe_id);const recipeDisabled=p.product_type==="recipe_product"&&!p.recipe_id&&p.inventory_policy==="none";setRecipeQuery(rp?.name||"");setForm({...p,cost:p.inventory_items?.average_cost??0,opening_quantity:"",supplier_id:p.preferred_supplier_id||"",warehouse_id:"",sale_unit:p.sale_unit||"piece",product_type:p.product_type||"non_stock",inventory_policy:p.product_type==="recipe_product"?(recipeDisabled?"none":"recipe_on_sale"):p.inventory_policy||"none",purchasable:p.product_type==="recipe_product"?false:p.purchasable??p.inventory_items?.purchasable??false,section_id:p.section_id||"",recipe_disabled:recipeDisabled});setOpen(true);setMenuId(null);}
  function changeType(t:string){const n:any={...form,product_type:t,recipe_id:null,sub_recipe_id:null,recipe_disabled:false};if(t==="raw_material"){n.inventory_policy="stock";n.show_on_cashier=false;n.purchasable=true;setRecipeQuery("");}else if(t==="purchased_product"){n.inventory_policy="stock";n.show_on_cashier=true;n.purchasable=true;setRecipeQuery("");}else if(t==="recipe_product"){n.inventory_policy="recipe_on_sale";n.show_on_cashier=true;n.purchasable=false;setRecipeQuery(String(form.name||""));}else if(t==="sub_recipe"){n.inventory_policy="none";n.show_on_cashier=false;n.purchasable=false;setRecipeQuery("");}else{n.inventory_policy="none";n.show_on_cashier=true;n.purchasable=false;setRecipeQuery("");}setRecipeBuilder(false);setForm(n);}
  function changeProductName(name:string){
    const next:any={...form,name};
    if(form.product_type==="recipe_product"&&!form.recipe_disabled){
  ...
  async function saveProduct(){
    if(!String(form.name||"").trim())return alert(L("Item name is required","اسم الصنف مطلوب"));
    if(form.product_type==="recipe_product"&&!form.recipe_disabled&&!form.recipe_id)return alert(L("Choose an existing recipe, add a new recipe, or disable recipe for this product.","اختر وصفة موجودة أو أضف وصفة جديدة أو عطّل الوصفة لهذا المنتج."));
    setBusy(true);
    ...
    let saveUrl="/api/commerce-admin",saveKind="product_save";
    if(form.product_type==="recipe_product"){
      payload.purchasable=false;
      if(form.recipe_disabled){payload.recipe_id=null;payload.inventory_policy="none";saveUrl="/api/product-recipe-optional";saveKind="save_without_recipe";}
      else payload.inventory_policy="recipe_on_sale";
    }
    if(imageBlob){payload.image_url=form.id?form.image_url||null:null;payload.image_path=form.image_path||null;}
    const j=await post(saveUrl,saveKind,payload);
  ...
  async function archiveProduct(id:string){if(!confirm(L("Archive this item? Previous invoices remain saved.","أرشفة هذا الصنف؟ الفواتير السابقة ستبقى محفوظة.")))return;setBusy(true);const j=await post("/api/product-data","archive",{id});if(j){setMenuId(null);setOpen(false);await load();}setBusy(false);}
  ...
  async function saveInlineRecipe(){
    ...
    if(j?.record){setForm((v:any)=>({...v,recipe_id:j.record.id,inventory_policy:"recipe_on_sale",purchasable:false,recipe_disabled:false}));setRecipeQuery(j.record.name);setRecipeBuilder(false);setRecipeDraft({name:"",lines:[]});await load();}
    setBusy(false);
  }
  ...
  const recipeOff=form.product_type==="recipe_product"&&form.recipe_disabled===true;
  const inventoryLogic=recipeOff?L("Recipe is disabled. This product can be sold without consuming recipe ingredients.","الوصفة معطّلة. يمكن بيع المنتج بدون استهلاك مكونات وصفة."):form.product_type==="recipe_product"?L("The selected recipe ingredients are consumed automatically when this product is sold.","يتم استهلاك مكونات الوصفة المختارة تلقائياً عند بيع هذا المنتج."):form.inventory_policy==="produced_stock"?L("Production adds finished stock; Cashier later decreases the finished item.","الإنتاج يضيف مخزوناً نهائياً ثم الكاشير يخصم المنتج النهائي."):form.inventory_policy==="stock"?L("The product is linked to Inventory and Cashier decreases its stock when sold.","المنتج مربوط بالمخزون والكاشير يخصم من رصيده عند البيع."):L("No inventory movement. The item can be sold without stock tracking.","بدون حركة مخزون ويمكن بيع الصنف من دون تتبع الكمية.");
  const accountingLogic=recipeOff?L("Revenue and VAT only until a recipe is linked; no recipe COGS is posted.","إيراد وضريبة فقط حتى يتم ربط وصفة؛ لا يتم ترحيل تكلفة وصفة."):form.product_type==="recipe_product"||tracked?L("Sale can post COGS against inventory according to its stock/recipe logic.","البيع يرحّل تكلفة البضاعة مقابل المخزون حسب منطق المخزون/الوصفة."):L("Revenue and VAT only; no Inventory Asset / COGS movement.","إيراد وضريبة فقط دون حركة أصل مخزون أو تكلفة بضاعة مباعة.");
  const sectionName=(id:any)=>d.sections.find((s:any)=>s.id===id)?.name||L("No section","بدون قسم");

  return <div className="space-y-5">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><h1 className="text-3xl font-black">{L("Add Items","إضافة الأصناف")}</h1><p className="text-sm text-slate-400">{L("Create each item once and connect it to Cashier, Inventory, Suppliers, Purchasing, Recipes and Accounting.","أضف كل صنف مرة واحدة واربطه بالكاشير والمخزون والموردين والمشتريات والوصفات والمحاسبة.")}</p></div><div className="flex flex-wrap gap-2"><button className={ghost} onClick={()=>setCatOpen(true)}>{L("Manage Categories","إدارة التصنيفات")}</button><button className={ghost} onClick={()=>setSectionOpen(true)}>{L("Manage Sections","إدارة الأقسام")}</button><button className={btn} onClick={newProduct}>+ {L("Item","صنف")}</button></div></div>

    <div className={card}>{d.products.length===0?<p className="text-slate-500">{L("No items yet.","لا توجد أصناف بعد.")}</p>:d.products.map((p:any)=><div key={p.id} className="relative grid gap-3 border-b border-slate-800 py-3 md:grid-cols-[70px_1fr_125px_105px_120px_44px] md:items-center">
    ...
    {open&&<Modal title={form.id?L("Edit Product","تعديل المنتج"):L("New Item","صنف جديد")} onClose={()=>{resetImage();resetRecipe();setOpen(false);}}><div className="space-y-4">
      ...
      {form.product_type==="recipe_product"&&<div className="space-y-3 rounded-2xl border border-cyan-500/20 bg-cyan-500/5 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3"><div><b>{L("Main Recipe","الوصفة الرئيسية")}</b><p className="text-xs text-slate-500">{L("Use a recipe when this product should consume ingredients on sale.","استخدم وصفة عندما يجب أن يستهلك المنتج مكونات عند البيع.")}</p></div><label className="flex cursor-pointer items-center gap-2 rounded-xl border border-slate-700 bg-slate-950 px-3 py-2 text-sm font-bold"><input type="checkbox" checked={form.recipe_disabled===true} onChange={e=>{const off=e.target.checked;setRecipeBuilder(false);setForm({...form,recipe_disabled:off,recipe_id:off?null:form.recipe_id,inventory_policy:off?"none":"recipe_on_sale",purchasable:false});if(off)setRecipeQuery("");else setRecipeQuery(String(form.name||""));}}/>{L("No Recipe / Disable","بدون وصفة / تعطيل")}</label></div>
        {form.recipe_disabled?<div className="rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-3 text-sm text-amber-200">{L("Recipe is disabled for this product. It can be saved and sold without a recipe and without consuming recipe ingredients.","الوصفة معطّلة لهذا المنتج. يمكن حفظه وبيعه بدون وصفة وبدون استهلاك مكونات وصفة.")}</div>:<>
          ...
          <div className="max-h-44 overflow-auto rounded-xl border border-slate-800 bg-slate-950 p-2">
            {recipeResults.length?recipeResults.map((r:any)=><button type="button" key={r.id} className={`mb-1 block w-full rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-800 rtl:text-right ${form.recipe_id===r.id?"bg-cyan-500/10 text-cyan-300":""}`} onClick={()=>{setForm({...form,recipe_id:r.id,inventory_policy:"recipe_on_sale",purchasable:false,recipe_disabled:false});setRecipeQuery(r.name);}}><b>{r.name}</b><small className="ml-2 text-slate-500 rtl:mr-2">{r.recipe_code}</small></button>):<p className="px-3 py-2 text-sm text-slate-500">{L("No matching recipe. Use Add New Recipe or disable recipe for this product.","لا توجد وصفة مطابقة. استخدم إضافة وصفة جديدة أو عطّل الوصفة لهذا المنتج.")}</p>}
          </div>
