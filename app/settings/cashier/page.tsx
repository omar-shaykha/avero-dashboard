...
export default function CashierMonitorSettingsPage(){
  ...
  return <div className="min-h-screen bg-slate-950 text-white">
    ...
    <div className="ml-64 flex min-h-screen flex-col"><DashboardHeader/><main className="flex-1 px-6 py-7"><div className="mx-auto max-w-6xl space-y-6">
      <div><div className="text-xs font-black uppercase tracking-[.2em] text-cyan-400">AVERO SETTINGS</div><h1 className="mt-2 text-3xl font-black">{L("Cashier Management","إدارة الكاشير")}</h1><p className="mt-2 text-sm text-slate-400">{L("Manage payment methods, cashier categories, visible items, Best Sellers and operational sections from one place.","إدارة طرق الدفع وتصنيفات الكاشير والأصناف الظاهرة والأكثر مبيعاً والأقسام التشغيلية من مكان واحد.")}</p><nav className="mt-4 flex flex-wrap gap-2"><Link className={btn} href="/settings/cashier">{L("Cashier Setup","إعداد الكاشير")}</Link><Link className={ghost} href="/settings/pos">{L("Receipts & Printers","الإيصالات والطابعات")}</Link></nav></div>
