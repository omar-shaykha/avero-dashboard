...
const defaults: NotificationPrefs = {
  master:true, sound:true, new_lead:true, qualified_lead:true, quotation_request:true,
  negotiation_started:true, high_interest:true, new_customer_message:true, won_deal:true,
  lost_deal:false, event_soon:true, ai_handoff:true, automation_error:true,
  subscription_expiry:true, security_alert:true,
};
...
export default function SettingsPage() {
  ...
  useEffect(()=>{
    try { const raw=localStorage.getItem("avero-notifications"); if(raw) setPrefs({...defaults,...JSON.parse(raw)}); } catch {}
  },[]);

  const update = (key:keyof NotificationPrefs,value:boolean) => {
    setPrefs(prev=>{ const next={...prev,[key]:value}; localStorage.setItem("avero-notifications",JSON.stringify(next)); return next; });
    setSaved(true); setTimeout(()=>setSaved(false),1400);
  };

  const ThemeCard = ({value,icon:Icon,title,desc}:{value:"dark"|"light"|"system";icon:typeof Moon;title:string;desc:string}) => (
    <button onClick={()=>setTheme(value)} className={`rounded-2xl border p-5 text-start transition ${theme===value?"border-blue-500 bg-blue-500/10":"border-slate-800 bg-slate-900/55 hover:border-slate-700"}`}>
      <div className="flex items-start justify-between"><div className="rounded-xl border border-slate-700 bg-slate-950 p-2.5"><Icon size={20} className="text-blue-400"/></div>{theme===value&&<Check size={18} className="text-emerald-400"/>}</div>
      <h3 className="mt-4 font-semibold text-white">{title}</h3><p className="mt-1 text-xs leading-5 text-slate-500">{desc}</p>
    </button>
  );
  ...
  return <div className="min-h-screen bg-slate-950"><Sidebar/><div className="ml-64 flex min-h-screen flex-col"><DashboardHeader/><main className="flex-1 px-6 py-7 text-white"><div className="mx-auto max-w-6xl space-y-7">
    <div className="flex items-center justify-between gap-4"><div><div className="mb-2 flex items-center gap-2 text-xs font-semibold uppercase tracking-[.2em] text-blue-400"><Settings2 size={15}/>{ar?"إعدادات AVERO":"AVERO SETTINGS"}</div><h1 className="text-3xl font-bold">{ar?"الإعدادات":"Settings"}</h1><p className="mt-2 text-sm text-slate-400">{ar?"تحكّم بالمظهر والتنبيهات وطريقة عمل حسابك.":"Control appearance, notifications and account behavior."}</p></div>{saved&&<div className="rounded-full border border-emerald-500/20 bg-emerald-500/10 px-4 py-2 text-xs text-emerald-300">{ar?"تم الحفظ":"Saved"}</div>}</div>
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-900/55 p-6"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-5"><div className="flex items-start gap-3"><div className="rounded-xl bg-violet-500/10 p-2.5 text-violet-300"><Bell size={21}/></div><div><h2 className="text-xl font-semibold">{ar?"التنبيهات":"Notifications"}</h2><p className="mt-1 text-sm text-slate-500">{ar?"اختر الأحداث التي تريد من AVERO أن ينبهك عليها.":"Choose exactly which events AVERO should notify you about."}</p></div></div><Toggle checked={prefs.master} onChange={v=>update("master",v)} label={ar?"تشغيل كل التنبيهات":"All notifications"}/></div>
      ...
      <div className="mt-5 flex gap-3 rounded-2xl border border-amber-500/15 bg-amber-500/5 p-4 text-xs leading-5 text-amber-200/80"><CircleAlert size={17} className="mt-0.5 shrink-0"/><p>{ar?"إعدادات التنبيهات تحفظ حالياً على الجهاز. عند مرحلة Supabase سنربطها بحساب العميل حتى تبقى محفوظة على كل الأجهزة، ونوصلها بمحرك التنبيهات الحقيقي داخل AVERO.":"Notification preferences are currently saved on this device. During the Supabase phase they will be synced to the client account across devices and connected to AVERO's live notification engine."}</p></div>
    </section>
  </div></main></div></div>;
}

function Toggle({checked,onChange,label}:{checked:boolean;onChange:(v:boolean)=>void;label?:string}) {
  return <button type="button" onClick={()=>onChange(!checked)} className="flex items-center gap-3"><span className="text-xs text-slate-400">{label}</span><span className={`relative h-7 w-12 rounded-full transition ${checked?"bg-emerald-500":"bg-slate-700"}`}><span className={`absolute top-1 h-5 w-5 rounded-full bg-white transition ${checked?"left-6":"left-1"}`}/></span></button>;
}
