...
type Product = { id: string; name: string; description: string | null; image_url: string | null; price: number; category_id: string | null; tax_enabled: boolean; tax_rate: number };
type Menu = { company: string; branch: string; pickup_address: string; prep_minutes: number; currency: string; prices_include_tax: boolean; categories: { id: string; name: string }[]; products: Product[] };
...
export default function PickupMenu() {
  ...
  return <main dir="rtl" className="min-h-screen bg-[#09111c] text-white">
    <div className="border-b border-amber-400/20 bg-[#0f1c2a] px-5 py-4"><div className="mx-auto flex max-w-6xl items-center justify-between"><span className="text-xl font-black tracking-wide text-amber-300">{menu?.company || "AVERO GO"}</span><span className="text-xs font-bold tracking-[.2em] text-slate-400">AVERO GO · PICKUP</span></div></div>
