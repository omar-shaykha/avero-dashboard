...
export interface AuthorizationContext {
  ...
  profile: { company_id: string | null; role: string | null; role_id: string | null };
  permissions: string[];
  features: string[];
}
...
export async function getAuthorizationContext(): Promise<AuthorizationContext | null> {
  ...
  if (profile?.role_id) {
    ...
    for (const row of data || []) {
      ...
      if (permission?.key) effective.set(permission.key, true);
    }
  }
  ...
  for (const row of overrides || []) {
    const permission = Array.isArray(row.permissions) ? row.permissions[0] : row.permissions;
    if (permission?.key) effective.set(permission.key, Boolean(row.allowed));
  }
  ...
  if (profile?.company_id) {
    ...
    for (const row of data || []) {
      ...
      if (feature?.key && (!row.expires_at || new Date(row.expires_at).getTime() > now)) features.push(feature.key);
    }
  }

  return {
    user: { id: user.id, email: user.email },
    profile: { company_id: profile?.company_id ?? null, role: profile?.role ?? null, role_id: profile?.role_id ?? null },
    permissions,
    features,
  };
}
...
export function hasPermission(context: AuthorizationContext | null, permissionKey: string) {
  return Boolean(isKingAdmin(context) || context?.permissions.includes(normalizePermissionKey(permissionKey)));
}
export function hasFeature(context: AuthorizationContext | null, featureKey: FeatureKey) {
  return Boolean(isKingAdmin(context) || context?.features.includes(featureKey));
}
export function canAccess(context: AuthorizationContext | null, featureKey: FeatureKey, permissionKey: string) {
  return Boolean(isKingAdmin(context) || (hasFeature(context, featureKey) && hasPermission(context, permissionKey)));
}
export function canManageCompanyUsers(context: AuthorizationContext | null) {
  return Boolean(isKingAdmin(context) || hasPermission(context, "users.permissions.manage"));
}
